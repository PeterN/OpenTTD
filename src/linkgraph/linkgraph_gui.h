...
public:
	...
	static const uint8_t LINK_COLOURS[];
	 ...
	 */
	LinkGraphOverlay(const Window *w, uint wid, CargoTypes cargo_mask, uint32_t company_mask, uint scale) :
			window(w), widget_id(wid), cargo_mask(cargo_mask), company_mask(company_mask), scale(scale)
	{}

	void Draw(const DrawPixelInfo *dpi);
	void SetCargoMask(CargoTypes cargo_mask);
	void SetCompanyMask(uint32_t company_mask);
	...
	uint32_t GetCompanyMask() { return this->company_mask; }
...
protected:
	...
	uint32_t company_mask;               ///< Bitmask of companies to be displayed.
