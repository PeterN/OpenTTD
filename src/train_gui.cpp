...
static int HighlightDragPosition(int px, int max_width, VehicleID selection, bool chain)
{
	...
	if (drag_hlight_width > 0) {
		GfxFillRect(drag_hlight_left + WD_FRAMERECT_LEFT, WD_FRAMERECT_TOP + 1,
				drag_hlight_right - WD_FRAMERECT_RIGHT, ScaleGUITrad(13) - WD_FRAMERECT_BOTTOM, GetColourGradient(COLOUR_GREY)[7]);
	}
			...
			for (uint i = 0; i < num_lines; i++) {
				...
				if (vscroll_pos <= 0 && vscroll_pos > -vscroll_cap) {
					...
					if (i > 0 || separate_sprite_row) {
						if (vscroll_pos != 0) GfxFillRect(left, py - WD_MATRIX_TOP - 1, right, py - WD_MATRIX_TOP, GetColourGradient(COLOUR_GREY)[5]);
					}
					switch (det_tab) {
						case TDW_TAB_CARGO:
