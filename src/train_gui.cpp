...
static int HighlightDragPosition(int px, int max_width, int y, VehicleID selection, bool chain)
{
	...
	if (drag_hlight_width > 0) {
		...
		GfxFillRect(r.Shrink(ScaleSpriteTrad(1)), ShadeColour(COLOUR_GREY, 7));
	}
			...
			for (uint i = 0; i < num_lines; i++) {
				if (vscroll_pos <= 0 && vscroll_pos > -vscroll_cap) {
					...
					if (i > 0 || separate_sprite_row) {
						if (vscroll_pos != 0) GfxFillRect(r.left, py - WidgetDimensions::scaled.matrix.top - 1, r.right, py - WidgetDimensions::scaled.matrix.top, ShadeColour(COLOUR_GREY, 5));
					}
					switch (det_tab) {
						case TDW_TAB_CARGO:
