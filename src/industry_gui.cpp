...
protected:
	...
	{
		ArrayStringParametersWriter<17> params;
		const IndustrySpec *indsp = GetIndustrySpec(i->type);
		uint8_t p = 0;

		/* Industry name */
		params.SetParam(p++, i->index);
		...
		for (size_t j = 0; j < std::min<size_t>(3, cargos.size()); j++) {
			...
			params.SetParam(p++, STR_INDUSTRY_DIRECTORY_ITEM_INFO);
			params.SetParam(p++, ci.cargo_id);
			params.SetParam(p++, ci.production);
			params.SetParam(p++, std::move(ci.suffix));
			params.SetParam(p++, ci.transported);
		}

		/* Undisplayed cargos if any */
		params.SetParam(p++, cargos.size() - 3);
