...
protected:
	 ...
	 */
	static void SetLastCommand(TileIndex tile, uint32_t p1, uint32_t p2, uint cmd);

	/**
	 * Check if it's the latest command executed by the script.
	 */
	static bool CheckLastCommand(TileIndex tile, uint32_t p1, uint32_t p2, uint cmd);
