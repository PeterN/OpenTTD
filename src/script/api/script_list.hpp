...
public:
	typedef std::set<int64_t> ScriptItemList;                   ///< The list of items inside the bucket
	typedef std::map<int64_t, ScriptItemList> ScriptListBucket; ///< The bucket list per value
	typedef std::map<int64_t, int64_t> ScriptListMap;             ///< List per item
	 ...
	 */
	void AddItem(int64_t item, int64_t value);
#else
	void AddItem(int64_t item, int64_t value = 0);
#endif /* DOXYGEN_API */
	 ...
	 */
	bool SetValue(int64_t item, int64_t value);
	 ...
	 */
	void RemoveBetweenValue(int64_t start, int64_t end);
	 ...
	 */
	void RemoveTop(int32_t count);

	/**
	 * Remove the last count items.
	 * @param count the amount of items to remove.
	 */
	void RemoveBottom(int32_t count);
