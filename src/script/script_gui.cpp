...
struct ScriptSettingsWindow : public Window {
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		for (auto it = first; it != last; ++it) {
			...
			if ((config_item.flags & SCRIPTCONFIG_BOOLEAN) != 0) {
				...
				*pi++ = current_value == 0 ? STR_CONFIG_SETTING_OFF : STR_CONFIG_SETTING_ON;
				...
				if (config_iterator != config_item.labels.end()) {
					*pi++ = STR_JUST_RAW_STRING;
					*pi++ = config_iterator->second;
				} else {
					*pi++ = STR_JUST_INT;
					*pi++ = current_value;
				}
			...
			DrawString(tr.left, tr.right, y + text_y_offset, GetStringWithArgs(str, {params.begin(), pi}), colour);
			y += this->line_height;
		}
	}
...
struct ScriptDebugWindow : public Window {
	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		...
		if (this->filter.script_debug_company == INVALID_COMPANY || !Company::IsValidAiID(this->filter.script_debug_company)) {
