...
struct DepotWindow : Window {
	...
	{
		if (widget == WID_D_CAPTION) return GetString(stringid, this->type, this->GetDepotIndex());
		...
		switch (widget) {
			...
			case WID_D_SELL_ALL:
				...
				if (!this->vehicle_list.empty() || !this->wagon_list.empty()) {
					ShowQuery(
						GetEncodedString(STR_DEPOT_CAPTION, this->type, this->GetDepotIndex()),
	...
	bool OnRightClick([[maybe_unused]] Point pt, WidgetID widget) override
	{
		...
		GuiShowTooltips(this,
			GetEncodedString(whole_chain ? STR_DEPOT_VEHICLE_TOOLTIP_CHAIN : STR_DEPOT_VEHICLE_TOOLTIP,
				whole_chain ? num : v->engine_type,
				details),
			TCC_RIGHT_CLICK);
