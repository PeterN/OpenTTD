...
/* static */ ParagraphLayouter *CoreTextParagraphLayoutFactory::GetParagraphLayout(CharType *buff, CharType *buff_end, FontMap &fontMapping)
{
	...
	for (const auto &i : fontMapping) {
		...
		if (font == nullptr) {
			if (!_font_cache[i.second->fc->GetIndex()]) {
				...
				_font_cache[i.second->fc->GetIndex()].reset(CTFontCreateWithName(font_name.get(), i.second->fc->GetFontSize(), nullptr));
			}
			font = _font_cache[i.second->fc->GetIndex()].get();
		}
		...
		if (i.second->fc->IsBuiltInFont()) {
			for (ssize_t c = last; c < i.first; c++) {
				CFAutoRelease<CTRunDelegateRef> del(CTRunDelegateCreate(&_custom_font_callback, static_cast<void *>(reinterpret_cast<uintptr_t>(buff[c] | (i.second->fc->GetIndex() << 24)))));
