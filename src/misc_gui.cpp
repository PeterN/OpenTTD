	...
	void OnInit() override
	{
		...
		for (uint i = 0; i < 4; i++) {
			...
			if (td.owner[i] != OWNER_NONE && td.owner[i] != OWNER_WATER) {
				...
				this->landinfo_data.push_back(GetStringWithArgs(td.owner_type[i], GetParamsForOwnedBy(td.owner[i], tile)));
			}
		}
...
struct AboutWindow : public Window {
	...
	{
		if (widget == WID_A_WEBSITE) param.SetParam(0, "Website: https://www.openttd.org");
		if (widget == WID_A_COPYRIGHT) param.SetParam(0, _openttd_revision_year);
	}
...
struct QueryStringWindow : public Window
{
	...
	void SetStringParameters(WidgetID widget, WidgetStringParameters &param) const override
	{
		if (widget == WID_QS_CAPTION) param.SetParam(0, this->editbox.caption);
	}
...
struct QueryWindow : public Window {
	...
	void SetStringParameters(WidgetID widget, WidgetStringParameters &param) const override
	{
		switch (widget) {
			case WID_Q_CAPTION:
				param.SetParam(0, this->caption.GetDecodedString());
				break;
		}
	}
...
static constexpr NWidgetPart _nested_query_widgets[] = {
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_CAPTION, COLOUR_RED, WID_Q_CAPTION), SetDataTip(STR_JUST_RAW_STRING, STR_TOOLTIP_WINDOW_TITLE_DRAG_THIS),
	EndContainer(),
