...
bool DrawStationTile(int x, int y, RailType railtype, Axis axis, StationClassID sclass, uint station)
{
	...
	if (HasBit(statspec->callback_mask, CBM_STATION_SPRITE_LAYOUT)) {
		uint16_t callback = GetStationCallback(CBID_STATION_SPRITE_LAYOUT, 0x2110000, 0, statspec, nullptr, INVALID_TILE);
	...
	for (TileIndex tile : area) {
		if (st->TileBelongsToRailStation(tile)) {
			...
			if (ss != nullptr && HasBit(ss->animation.triggers, trigger)) {
				...
				StationAnimationBase::ChangeAnimationFrame(CBID_STATION_ANIM_START_STOP, ss, st, tile, (random_bits << 16) | Random(), (uint8_t)trigger | (cargo << 8));
			}
		}
	}
}
