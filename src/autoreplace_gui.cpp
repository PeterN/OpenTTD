...
void DrawEngineList(VehicleType type, const Rect &r, const GUIEngineList &eng_list, int min, int max, EngineID selected_id, bool show_count, GroupID selected_group);
	...
	void DrawWidget(const Rect &r, int widget) const override
	{
		switch (widget) {
			...
			case WID_RV_RIGHT_MATRIX: {
				...
				int start = this->vscroll[side]->GetPosition(); // what is the offset for the start (scrolling)
				int end = static_cast<int>(std::min<size_t>(this->vscroll[side]->GetCapacity() + start, this->engines[side].size()));
