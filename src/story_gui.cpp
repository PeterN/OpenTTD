...
protected:
	...
	DropDownList BuildDropDownList() const
	{
		...
		for (const StoryPage *p : this->story_pages) {
			...
			if (!p->title.empty()) {
				list.push_back(MakeDropDownListStringItem(GetString(STR_JUST_RAW_STRING, p->title), p->index, current_page));
			} else {
				/* No custom title => use a generic page title with page number. */
				list.push_back(MakeDropDownListStringItem(GetString(STR_STORY_BOOK_GENERIC_PAGE_ITEM, page_num), p->index, current_page));
			}
			page_num++;
		}
