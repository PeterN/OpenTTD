...
/** CargoType wrapper to ensure that type is unique. */
struct CargoTypeTag {
	CargoType value;
	constexpr CargoTypeTag(CargoType cargo) : value(cargo) {}
};
...
public:
	 ...
	 */
	CargoDataEntry *InsertOrRetrieve(CargoTypeTag cargo)
	{
		return this->InsertOrRetrieve<CargoTypeTag>(cargo);
	}
	 ...
	 */
	void Remove(CargoTypeTag cargo)
	{
	 ...
	 */
	CargoDataEntry *Retrieve(CargoTypeTag cargo) const
	{
		CargoDataEntry t(cargo);
		return this->Retrieve(this->children->find(&t));
	}
	 ...
	 */
	CargoType GetCargo() const { return this->cargo.value; }
...
private:
	...
	CargoDataEntry(CargoTypeTag car, uint c, CargoDataEntry *p);
	CargoDataEntry(StationIDTag st);
	CargoDataEntry(CargoTypeTag car);
	...
	CargoDataEntry *parent = nullptr; ///< the parent of this entry.
	StationIDTag station = INVALID_STATION; ///< ID of the station this entry is associated with.
	CargoTypeTag cargo = INVALID_CARGO; ///< ID of the cargo this entry is associated with.
	bool transfers = false; ///< If there are transfers for this cargo.
	uint num_children = 0; ///< the number of subentries belonging to this entry.
	uint count = 0; ///< sum of counts of all children or amount of cargo for this entry.
	CargoDataSet *children = nullptr; ///< the children of this entry.
};
...
CargoDataEntry::CargoDataEntry(CargoTypeTag cargo, uint count, CargoDataEntry *parent) :
	parent(parent),
	cargo(cargo),
	count(count),
...
CargoDataEntry::CargoDataEntry(CargoTypeTag cargo) :
	cargo(cargo)
{}
...
bool CargoSorter::operator()(const CargoDataEntry *cd1, const CargoDataEntry *cd2) const
{
	switch (this->type) {
		...
		case CargoSortType::CargoType:
			return this->SortId<CargoTypeTag>(cd1->GetCargo(), cd2->GetCargo());
...
struct StationViewWindow : public Window {
	 ...
	 */
	struct RowDisplay {
		RowDisplay(CargoDataEntry *f, StationID station, CargoType cargo) : filter(f), next_station(station), next_cargo(cargo) {}
	...
	void ShowCargo(CargoDataEntry *data, CargoType cargo, StationID source, StationID next, StationID dest, uint count)
	{
		...
		for (int i = 0; i < NUM_COLUMNS && expand != nullptr; ++i) {
			switch (groupings[i]) {
				case GR_CARGO:
					assert(i == 0);
					data = data->InsertOrRetrieve(CargoTypeTag{cargo});
					data->SetTransfers(source != this->window_number);
					expand = expand->Retrieve(CargoTypeTag{cargo});
					break;
				case GR_SOURCE:
	...
	{
		const Station *st = Station::Get(this->window_number);
		CargoDataEntry *cargo_entry = cached_destinations.InsertOrRetrieve(CargoTypeTag{i});
		cargo_entry->Clear();
	...
	{
		const CargoDataEntry *source_dest = this->cached_destinations.Retrieve(CargoTypeTag{i});
	...
	void SetDisplayedRow(const CargoDataEntry *data)
	{
		...
		if (parent->GetParent() == nullptr) {
			this->displayed_rows.emplace_back(&this->expanded_rows, INVALID_STATION, data->GetCargo());
			return;
		}
		...
		CargoDataEntry *filter = this->expanded_rows.Retrieve(CargoTypeTag{cargo});
		while (!stations.empty()) {
			filter = filter->Retrieve(StationIDTag{stations.back()});
			stations.pop_back();
		}

		this->displayed_rows.emplace_back(filter, next, INVALID_CARGO);
	}
	...
	void HandleCargoWaitingClick(int row)
	{
		...
		if (_ctrl_pressed) {
			...
			if (display.filter == &this->expanded_rows) {
				this->HandleCargoWaitingClick<CargoTypeTag>(display.filter, display.next_cargo);
			} else {
