...
int DrawStationCoverageAreaText(int left, int right, int top, StationCoverageType sct, int rad, bool supplies)
{
	...
	if (_thd.drawstyle == HT_RECT && tile < Map::Size()) {
		...
		for (auto &[cid, amount] : cargoes.amounts) {
			switch (sct) {
				case SCT_PASSENGERS_ONLY: if (!IsCargoInClass(cid, CC_PASSENGERS)) continue; break;
				case SCT_NON_PASSENGERS_ONLY: if (IsCargoInClass(cid, CC_PASSENGERS)) continue; break;
				case SCT_ALL: break;
				default: NOT_REACHED();
			}
			if (amount >= (supplies ? 1U : 8U)) SetCargo(cargo_mask, cid);
		}
	}
...
protected:
		...
		for (const Station *st : Station::Iterate()) {
			if ((this->filter.facilities & st->facilities) != 0) { // only stations with selected facilities
				if (st->owner == owner || (st->owner == OWNER_NONE && HasStationInUse(st->index, true, owner))) {
					...
					for (CargoID j = 0; j < CargoSpec::Count(); j++) {
	...
	static bool StationWaitingTotalSorter(const Station * const &a, const Station * const &b, const CargoTypes &cargo_filter)
	{
		...
		for (const CargoID &j : cargo_filter) {
...
struct StationViewWindow : public Window {
	...
	{
		for (CargoID i = 0; i < CargoSpec::Count(); i++) {
