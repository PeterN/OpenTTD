...
static bool RoadToolbar_CtrlChanged(Window *w)
{
	...
	for (WidgetIndex i = WID_ROT_ROAD_X; i <= WID_ROT_AUTOROAD; i++) {
...
static const NWidgetPart _nested_build_road_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_PANEL, COLOUR_DARK_GREEN), SetMinimalSize(0, 22), SetFill(1, 1), EndContainer(),
...
struct BuildRoadDepotWindow : public PickerWindowBase {
	BuildRoadDepotWindow(WindowDesc *desc, Window *parent) : PickerWindowBase(desc, parent)
	{
		...
		if (RoadTypeIsTram(_cur_roadtype)) {
			...
			for (WidgetIndex i = WID_BROD_DEPOT_NE; i <= WID_BROD_DEPOT_NW; i++) this->GetWidget<NWidgetCore>(i)->tool_tip = STR_BUILD_DEPOT_TRAM_ORIENTATION_SELECT_TOOLTIP;
		}
...
public:
	BuildRoadStationWindow(WindowDesc *desc, Window *parent, RoadStopType rs) : PickerWindowBase(desc, parent), filter_editbox(EDITBOX_MAX_SIZE * MAX_CHAR_LENGTH, EDITBOX_MAX_SIZE)
	{
		...
		for (WidgetIndex i = RoadTypeIsTram(_cur_roadtype) ? WID_BROS_STATION_X : WID_BROS_STATION_NE; i < WID_BROS_LT_OFF; i++) {
