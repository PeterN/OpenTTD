	...
	if (v->state < 2) {
		...
		if (v->state == 1) {
			...
			if (IsValidTile(v->tile) && IsAirportTile(v->tile)) {
				...
				AddTileNewsItem(GetEncodedString(STR_NEWS_DISASTER_ZEPPELIN, GetStationIndex(v->tile)), NT_ACCIDENT, v->tile);
	...
	if (v->state == 0) {
		...
		if (z <= u->z_pos && (u->vehstatus & VS_HIDDEN) == 0) {
			...
			if (u->crashed_ctr == 0) {
				...
				AddTileNewsItem(GetEncodedString(STR_NEWS_DISASTER_SMALL_UFO), NT_ACCIDENT, u->tile);
...
static bool DisasterTick_Aircraft(DisasterVehicle *v, uint16_t image_override, bool leave_at_top, StringID news_message, IndustryBehaviour industry_flag)
{
	...
	if (v->state == 2) {
		...
		if (++v->age == 112) {
			...
			AddIndustryNewsItem(GetEncodedString(news_message, i->town->index), NT_ACCIDENT, i->index);
...
static bool DisasterTick_Big_Ufo(DisasterVehicle *v)
{
	...
	if (v->state == 1) {
		...
		AddTileNewsItem(GetEncodedString(STR_NEWS_DISASTER_BIG_UFO, t->index), NT_ACCIDENT, v->tile);
	...
	for (m = 0; m < 15; m++) {
		for (const Industry *i : Industry::Iterate()) {
			if ((GetIndustrySpec(i->type)->behaviour & INDUSTRYBEH_CAN_SUBSIDENCE) && --index < 0) {
				AddTileNewsItem(GetEncodedString(STR_NEWS_DISASTER_COAL_MINE_SUBSIDENCE, i->town->index), NT_ACCIDENT, i->location.tile + TileDiffXY(1, 1)); // keep the news, even when the mine closes
