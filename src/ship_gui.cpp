...
void DrawShipDetails(const Vehicle *v, const Rect &r)
{
	...
	DrawString(r.left, r.right, y,
			GetString(STR_VEHICLE_INFO_BUILT_VALUE, PackEngineNameDParam(v->engine_type, EngineNameContext::VehicleDetails), v->build_year, v->value));
	y += GetCharacterHeight(FS_NORMAL);

	DrawString(r.left, r.right, y,
			GetString(STR_VEHICLE_INFO_CAPACITY, v->cargo_type, v->cargo_cap, std::monostate{}, std::monostate{}, GetCargoSubtypeText(v)));
	y += GetCharacterHeight(FS_NORMAL) + WidgetDimensions::scaled.vsep_normal;

	if (v->cargo.StoredCount() > 0) {
		DrawString(r.left, r.right, y,
				GetString(STR_VEHICLE_DETAILS_CARGO_FROM, v->cargo_type, v->cargo.StoredCount(), v->cargo.GetFirstStation()));
	} else {
