...
void ShowNewGRFError()
{
	...
	for (const GRFConfig *c = _grfconfig; c != nullptr; c = c->next) {
		...
		std::array<StringParameter, 5 + std::tuple_size_v<decltype(c->error->param_value)>> params{};
		auto pi = params.begin();
		*pi++ = c->GetName();
		*pi++ = c->error->message != STR_NULL ? c->error->message : STR_JUST_RAW_STRING;
		*pi++ = c->error->custom_message;
		*pi++ = c->filename;
		*pi++ = c->error->data;
		for (uint i = 0; i < c->error->param_value.size(); i++) {
			*pi++ = c->error->param_value[i];
		}
		if (c->error->severity == STR_NEWGRF_ERROR_MSG_FATAL) {
			ShowErrorMessage(GetEncodedStringWithArgs(STR_NEWGRF_ERROR_FATAL_POPUP, {params.begin(), pi}), {}, WL_CRITICAL);
		} else {
			ShowErrorMessage(GetEncodedStringWithArgs(STR_NEWGRF_ERROR_POPUP, {params.begin(), pi}), {}, WL_ERROR);
		}
		break;
	}
}
...
static void ShowNewGRFInfo(const GRFConfig *c, const Rect &r, bool show_params)
{
	...
	if (c->error.has_value()) {
		...
		auto pi = params.begin();
		*pi++ = c->error->custom_message; // is skipped by built-in messages
		*pi++ = c->filename;
		*pi++ = c->error->data;
		for (uint i = 0; i < c->error->param_value.size(); i++) {
			*pi++ = c->error->param_value[i];
		...
		tr.top = DrawStringMultiLine(tr, GetString(c->error->severity, GetStringWithArgs(c->error->message != STR_NULL ? c->error->message : STR_JUST_RAW_STRING, {params.begin(), pi})));
	}
					...
					if (this->clicked_dropdown) {
						...
						if (pt.y >= wi_rect.top && pt.y <= wi_rect.bottom) {
							...
							for (uint32_t i = par_info.min_value; i <= par_info.max_value; i++) {
								list.push_back(MakeDropDownListStringItem(GetString(STR_JUST_RAW_STRING, GetGRFStringFromGRFText(par_info.value_names.find(i)->second)), i));
							}
