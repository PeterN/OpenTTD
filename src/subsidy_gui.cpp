...
struct SubsidyListWindow : Window {
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		const bool wallclock = TimerGameEconomy::UsingWallclockUnits();
		...
		for (const Subsidy *s : Subsidy::Iterate()) {
			if (!s->IsAwarded()) {
				if (IsInsideMM(pos, 0, cap)) {
					...
					std::string text = GetString(STR_SUBSIDIES_OFFERED_FROM_TO,
						cs->name, srcfmt, s->src, dstfmt, s->dst,
						wallclock ? STR_SUBSIDIES_OFFERED_EXPIRY_TIME : STR_SUBSIDIES_OFFERED_EXPIRY_DATE,
						wallclock ? s->remaining + 1 : TimerGameEconomy::date.base() - ymd.day + s->remaining * 32);
		...
		for (const Subsidy *s : Subsidy::Iterate()) {
			if (s->IsAwarded()) {
				if (IsInsideMM(pos, 0, cap)) {
					...
					std::string text = GetString(STR_SUBSIDIES_SUBSIDISED_FROM_TO,
						cs->name, srcfmt, s->src, dstfmt, s->dst, GetString(STR_COMPANY_NAME, s->awarded),
						wallclock ? STR_SUBSIDIES_SUBSIDISED_EXPIRY_TIME : STR_SUBSIDIES_SUBSIDISED_EXPIRY_DATE,
						wallclock ? s->remaining + 1 : TimerGameEconomy::date.base() - ymd.day + s->remaining * 32);
