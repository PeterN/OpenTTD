...
{
	ShowQueryString(GetString(STR_JUST_INT, _settings_game.game_creation.starting_year.base()), STR_MAPGEN_START_DATE_QUERY_CAPT, 8, w, CS_NUMERAL, QSF_ENABLE_DEFAULT);
	return CBF_NONE;
}
...
struct ScenarioEditorToolbarWindow : Window {
	...
	void SetStringParameters(WidgetID widget, WidgetStringParameters &param) const override
	{
		switch (widget) {
			case WID_TE_DATE:
				param.SetParam(0, TimerGameCalendar::ConvertYMDToDate(_settings_game.game_creation.starting_year, 0, 1));
				break;
		}
	}
