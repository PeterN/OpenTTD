...
std::array<StringParameter, 2> GetParamsForOwnedBy(Owner owner, TileIndex tile)
{
	...
	if (owner != OWNER_TOWN) {
		if (!Company::IsValidID(owner)) {
			params[0] = STR_COMPANY_SOMEONE;
			params[1] = std::monostate{};
		} else {
			params[0] = STR_COMPANY_NAME;
			params[1] = owner;
		}
	} else {
		assert(tile != 0);
...
set_name:;
		...
		if (c->is_ai) {
			...
			AddNewsItem(std::move(headline),
				NT_COMPANY_INFO, NF_COMPANY, NR_TILE, c->last_build_coordinate.base(), NR_NONE, UINT32_MAX, std::move(cni));
		}
		return;
	}
bad_town_name:;
