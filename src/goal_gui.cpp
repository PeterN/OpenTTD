...
struct GoalQuestionWindow : public Window {
	...
	GoalQuestionWindow(WindowDesc *desc, WindowNumber window_number, TextColour colour, uint32_t button_mask, const char *question) : Window(desc), colour(colour)
	{
 ...
 */
void ShowGoalQuestion(uint16_t id, byte type, uint32_t button_mask, const char *question)
{
	assert(type < GQT_END);
