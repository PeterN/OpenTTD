 ...
 */
template <typename T>
static inline uint GB(const T x, const uint8_t s, const uint8_t n)
{
	return (x >> s) & (((T)1U << n) - 1);
}
...
uint8_t FindFirstBit(uint32_t x);
...
#	define BSWAP32(x) (static_cast<uint32_t>(CFSwapint32_t(x)))
#	define BSWAP16(x) (static_cast<uint16_t>(CFSwapint16_t(x)))
