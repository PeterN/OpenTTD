...
std::string GenerateDefaultSaveName()
{
	...
	*pi++ = cid;
	...
	if (TimerGameEconomy::UsingWallclockUnits()) {
		...
		*pi++ = STR_SAVEGAME_DURATION_REALTIME;
		*pi++ = play_time / 60 / 60;
		*pi++ = (play_time / 60) % 60;
	} else {
		/* Insert current date */
		switch (_settings_client.gui.date_format_in_default_names) {
			case 0: *pi++ = STR_JUST_DATE_LONG; break;
			case 1: *pi++ = STR_JUST_DATE_TINY; break;
			case 2: *pi++ = STR_JUST_DATE_ISO; break;
			default: NOT_REACHED();
		}
		*pi++ = TimerGameEconomy::date;
	}

	/* Get the correct string (special string for when there's not company) */
	std::string filename = GetStringWithArgs(!Company::IsValidID(cid) ? STR_SAVEGAME_NAME_SPECTATOR : STR_SAVEGAME_NAME_DEFAULT, {params.begin(), pi});
