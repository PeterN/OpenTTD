...
struct SaveLoadParams {
	...
	uint16_t game_speed;                   ///< The game speed when saving started.
...
static inline int SlReadUint16_t()
{
	int x = SlReadByte() << 8;
	return x | SlReadByte();
}

static inline uint32_t SlReadUint32_t()
{
	uint32_t x = SlReadUint16_t() << 16;
	return x | SlReadUint16_t();
}

static inline uint64_t SlReadUint64_t()
{
	uint32_t x = SlReadUint32_t();
	uint32_t y = SlReadUint32_t();
	return (uint64_t)x << 32 | y;
}

static inline void SlWriteUint16_t(uint16_t v)
{
	SlWriteByte(GB(v, 8, 8));
	SlWriteByte(GB(v, 0, 8));
}

static inline void SlWriteUint32_t(uint32_t v)
{
	SlWriteUint16_t(GB(v, 16, 16));
	SlWriteUint16_t(GB(v,  0, 16));
}

static inline void SlWriteUint64_t(uint64_t x)
{
	SlWriteUint32_t((uint32_t)(x >> 32));
	SlWriteUint32_t((uint32_t)x);
}
		...
		case NL_WANTLENGTH:
			...
			switch (_sl.block_mode) {
				case CH_RIFF:
					...
					assert(length < (1 << 28));
					SlWriteUint32_t((uint32_t)((length & 0xFFFFFF) | ((length >> 24) << 28)));
					break;
				case CH_ARRAY:
	...
	switch (_sl.action) {
		case SLA_SAVE: {
			...
			switch (GetVarFileType(conv)) {
				...
				case SLE_FILE_I16:assert(x >= -32768 && x <= 32767); SlWriteUint16_t(x);break;
				case SLE_FILE_STRINGID:
				case SLE_FILE_U16:assert(x >= 0 && x <= 65535);      SlWriteUint16_t(x);break;
				case SLE_FILE_I32:
				case SLE_FILE_U32:                                   SlWriteUint32_t((uint32_t)x);break;
				case SLE_FILE_I64:
				case SLE_FILE_U64:                                   SlWriteUint64_t(x);break;
				default: NOT_REACHED();
			}
			break;
		}
		...
		case SLA_LOAD: {
			...
			switch (GetVarFileType(conv)) {
				...
				case SLE_FILE_I16: x = (int16_t )SlReadUint16_t(); break;
				case SLE_FILE_U16: x = (uint16_t)SlReadUint16_t(); break;
				case SLE_FILE_I32: x = (int32_t )SlReadUint32_t(); break;
				case SLE_FILE_U32: x = (uint32_t)SlReadUint32_t(); break;
				case SLE_FILE_I64: x = (int64_t )SlReadUint64_t(); break;
				case SLE_FILE_U64: x = (uint64_t)SlReadUint64_t(); break;
				case SLE_FILE_STRINGID: x = RemapOldStringID((uint16_t)SlReadUint16_t()); break;
				default: NOT_REACHED();
			}
	...
	if (_sl.action != SLA_SAVE && _sl_version == 0) {
		...
		if (conv == (SLE_FILE_I32 | SLE_VAR_I64)) {
			for (uint i = 0; i < length; i++) {
				((int64_t*)object)[i] = (int32_t)BSWAP32(SlReadUint32_t());
			}
			return;
		}
	}
...
void SlSaveLoadRef(void *ptr, VarType conv)
{
	switch (_sl.action) {
		case SLA_SAVE:
			SlWriteUint32_t((uint32_t)ReferenceToInt(*(void **)ptr, (SLRefType)conv));
			break;
		case SLA_LOAD_CHECK:
		case SLA_LOAD:
			*(size_t *)ptr = IsSavegameVersionBefore(SLV_69) ? SlReadUint16_t() : SlReadUint32_t();
			break;
		case SLA_PTRS:
	...
	static void SlSaveLoad(void *storage, VarType conv, SaveLoadType cmd = SL_VAR)
	{
		...
		switch (_sl.action) {
			...
			case SLA_LOAD: {
				size_t length;
				switch (cmd) {
					case SL_VAR: length = IsSavegameVersionBefore(SLV_SAVELOAD_LIST_LENGTH) ? SlReadUint32_t() : SlReadArrayLength(); break;
					case SL_REF: length = IsSavegameVersionBefore(SLV_69) ? SlReadUint16_t() : IsSavegameVersionBefore(SLV_SAVELOAD_LIST_LENGTH) ? SlReadUint32_t() : SlReadArrayLength(); break;
					default: NOT_REACHED();
				}
...
static void SlLoadChunks()
{
	...
	for (id = SlReadUint32_t(); id != 0; id = SlReadUint32_t()) {
