		...
		for (uint i = 0; i < chunk->amount; i++) {
			...
			if (GetOldChunkType(chunk->type) != 0) {
				...
				switch (GetOldChunkFileType(chunk->type)) {
					...
					case OC_FILE_I16: res = (int16_t)ReadUint16_t(ls); break;
					case OC_FILE_U16: res = ReadUint16_t(ls); break;
					case OC_FILE_I32: res = (int32_t)ReadUint32_t(ls); break;
					case OC_FILE_U32: res = ReadUint32_t(ls); break;
					default: NOT_REACHED();
				}
