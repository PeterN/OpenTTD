...
static const SaveLoad _cargomonitor_pair_desc[] = {
	...
	SLE_CONDVAR(TempStorage, cargo, SLE_FILE_U8 | SLE_VAR_U16, SLV_VARIABLE_CARGO_ARRAY, SLV_EXTEND_CARGOTYPES_MORE),
	SLE_CONDVAR(TempStorage, cargo, SLE_UINT16, SLV_EXTEND_CARGOTYPES_MORE, SL_MAX_VERSION),
	...
	void Load() const override
	{
		...
		for (;;) {
			...
			if (decode) {
				...
				storage.cargo = static_cast<CargoID>(GB(storage.number, OLD_CCB_CARGO_TYPE_START, OLD_CCB_CARGO_TYPE_LENGTH));
