...
{
	for (FontSize fs = FS_BEGIN; fs < FS_END; fs++) {
		auto *sfc = FontCache::GetSpriteFontCache(fs);
		assert(sfc != nullptr);
		sfc->InitializeUnicodeGlyphMap();
	}
}

void SetUnicodeGlyph(FontSize size, char32_t key, SpriteID sprite)
{
	FontCache::GetSpriteFontCache(size)->SetUnicodeGlyph(key, sprite);
}
...
{
	Debug(fontcache, 0, "InitFontCache (monospace {})", monospace);
