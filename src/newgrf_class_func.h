 ...
 * @return The spec.
 */
DEFINE_NEWGRF_CLASS_METHOD(const Tspec *)::GetByGrf(uint32_t grfid, byte local_id, int *index)
{
	uint j;
...
#define INSTANTIATE_NEWGRF_CLASS_METHODS(name, Tspec, Tid, Tmax) \
	...
	template const Tspec *name::GetByGrf(uint32_t grfid, byte localidx, int *index);
