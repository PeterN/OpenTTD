...
void ResetBadgeClassConfiguration(GrfSpecFeature feature)
{
	...
	for (const BadgeID &badgeid : GetClassBadges()) {
		config.emplace_back(GetBadge(badgeid)->label, 0, true);
	}
}
...
DropDownList BuildBadgeClassConfigurationList(const GUIBadgeClasses &badge_classes, uint columns, std::span<const StringID> column_separators)
{
	DropDownList list;
