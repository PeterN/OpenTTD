...
void AddNewsItem(StringID string, NewsType type, NewsFlag flags, NewsReferenceType reftype1 = NR_NONE, uint32_t ref1 = UINT32_MAX, NewsReferenceType reftype2 = NR_NONE, uint32_t ref2 = UINT32_MAX, std::unique_ptr<const NewsAllocatedData> &&data = nullptr);

inline void AddCompanyNewsItem(StringID string, std::unique_ptr<const CompanyNewsInformation> &&cni)
{
...
inline void AddTileNewsItem(StringID string, NewsType type, TileIndex tile, std::unique_ptr<const NewsAllocatedData> &&data = nullptr, StationID station = INVALID_STATION)
{
...
inline void AddIndustryNewsItem(StringID string, NewsType type, IndustryID industry, std::unique_ptr<const NewsAllocatedData> &&data = nullptr)
{
