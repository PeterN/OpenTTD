					...
					for (uint k = 0;; k++) {
						...
						if (it.gfx == 0xFE) {
							...
							/* When there were only 256x256 maps, TileIndex was a uint16_t and
								...
								*   x = -1, y = 1 -> x = 255, y = 0
								* Since GRF version 8 the position is interpreted as pair of independent int8_t.
...
{
	uint8_t *stations = AllocaM(uint8_t, idcount);
	for (uint i = 0; i < idcount; i++) {
...
{
	uint8_t *houses = AllocaM(uint8_t, idcount);
	for (uint i = 0; i < idcount; i++) {
...
{
	uint8_t *industries = AllocaM(uint8_t, idcount);
	for (uint i = 0; i < idcount; i++) {
...
{
	uint8_t *indtiles = AllocaM(uint8_t, idcount);
	for (uint i = 0; i < idcount; i++) {
...
static void ObjectMapSpriteGroup(ByteReader *buf, uint8_t idcount)
{
	...
	uint8_t *objects = AllocaM(uint8_t, idcount);
	for (uint i = 0; i < idcount; i++) {
...
{
	uint8_t *airports = AllocaM(uint8_t, idcount);
	for (uint i = 0; i < idcount; i++) {
...
{
	uint8_t *airptiles = AllocaM(uint8_t, idcount);
	for (uint i = 0; i < idcount; i++) {
...
static bool ChangeGRFParamValueNames(ByteReader *buf)
{
	...
	while (type != 0) {
		...
		std::pair<uint32_t, GRFTextList> *val_name = _cur_parameter->value_names.Find(id);
...
void LoadNewGRF(uint load_index, uint num_baseset)
{
	...
	uint16_t tick_counter  = _tick_counter;
