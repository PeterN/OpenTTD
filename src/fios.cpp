...
extern bool FiosGetDiskFreeSpace(const char *path, uint64_t *tot);
...
bool FiosFileScanner::AddFile(const std::string &filename, size_t basepath_length, const std::string &tar_filename)
{
	...
	if (fh != INVALID_HANDLE_VALUE) {
		FILETIME ft;
		ULARGE_INTEGER ft_int64_t;

		if (GetFileTime(fh, nullptr, nullptr, &ft) != 0) {
			ft_int64_t.HighPart = ft.dwHighDateTime;
			ft_int64_t.LowPart = ft.dwLowDateTime;

			// Convert from hectonanoseconds since 01/01/1601 to seconds since 01/01/1970
			fios->mtime = ft_int64_t.QuadPart / 10000000ULL - 11644473600ULL;
		} else {
			fios->mtime = 0;
		}
...
struct ScenarioIdentifier {
	...
	uint8_t md5sum[16];        ///< MD5 checksum of file.
