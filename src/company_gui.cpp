...
struct CompanyWindow : Window
{
	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		switch (widget) {
			...
			case WID_C_DESC_COMPANY_VALUE:
				return GetString(stringid, CalculateCompanyValue(Company::Get((CompanyID)this->window_number)));
...
struct BuyCompanyWindow : Window {
	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		switch (widget) {
			case WID_BC_CAPTION:
				return GetString(stringid, Company::Get((CompanyID)this->window_number)->index);
