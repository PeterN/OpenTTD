		...
		switch (widget) {
			...
			case WID_CL_CLIENT_NAME: {
				...
				if (widget == WID_CL_SERVER_NAME) {
					text = GetString(STR_JUST_RAW_STRING, _network_server ? _settings_client.network.server_name : _network_server_name);
				} else {
					const NetworkClientInfo *own_ci = NetworkClientInfo::GetByClientID(_network_own_client_id);
					text = GetString(STR_JUST_RAW_STRING, own_ci != nullptr ? own_ci->client_name : _settings_client.network.client_name);
				}
				size = GetStringBoundingBox(text);
	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		switch (widget) {
			...
			case WID_CL_SERVER_INVITE_CODE: {
				...
				return GetString(stringid, _network_server_connection_type == CONNECTION_TYPE_UNKNOWN ? empty : _network_server_invite_code);
			}
