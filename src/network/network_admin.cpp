...
NetworkRecvStatus ServerNetworkAdminSocketHandler::SendCompanyEconomy()
{
	for (const Company *company : Company::Iterate()) {
		...
		p->Send_uint16(static_cast<uint16>(std::min<uint64>(UINT16_MAX, GetCargoArraySum<OverflowSafeInt64>(company->cur_economy.delivered_cargo))));
		...
		for (uint i = 0; i < 2; i++) {
			...
			p->Send_uint16(static_cast<uint16>(std::min<uint64>(UINT16_MAX, GetCargoArraySum<OverflowSafeInt64>(company->old_economy[i].delivered_cargo))));
		}
