...
Packet::Packet(PacketType type, size_t limit) : next(nullptr), pos(0), limit(limit), cs(nullptr)
{
	...
	this->Send_uint16_t(0);
	this->Send_uint8_t(type);
}
 ...
 * @param data The data to send.
 */
void Packet::Send_uint8_t(uint8_t data)
{
 ...
 * @param data The data to send.
 */
void Packet::Send_uint16_t(uint16_t data)
{
	assert(this->CanWriteToPacket(sizeof(data)));
 ...
 * @param data The data to send.
 */
void Packet::Send_uint32_t(uint32_t data)
{
	assert(this->CanWriteToPacket(sizeof(data)));
	this->buffer.emplace_back(GB(data,  0, 8));
	this->buffer.emplace_back(GB(data,  8, 8));
 ...
 * @param data The data to send.
 */
void Packet::Send_uint64_t(uint64_t data)
{
	assert(this->CanWriteToPacket(sizeof(data)));
	this->buffer.emplace_back(GB(data,  0, 8));
	this->buffer.emplace_back(GB(data,  8, 8));
...
std::string Packet::Recv_string(size_t length, StringValidationSettings settings)
{
	...
	/* Both loops with Recv_uint8_t terminate when reading past the end of the
	 * packet as Recv_uint8_t then closes the connection and returns 0. */
	std::string str;
	char character;
	while (--length > 0 && (character = this->Recv_uint8_t()) != '\0') str.push_back(character);

	if (length == 0) {
		/* The string in the packet was longer. Read until the termination. */
		while (this->Recv_uint8_t() != '\0') {}
	}
