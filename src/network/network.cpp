	...
	switch (changed_mode) {
		...
		case PM_PAUSED_LINK_GRAPH: {
			...
			if (!changed) {
				...
				if ((_pause_mode & PM_PAUSED_NORMAL) != PM_UNPAUSED)         *pi++ = STR_NETWORK_SERVER_MESSAGE_GAME_REASON_MANUAL;
				if ((_pause_mode & PM_PAUSED_JOIN) != PM_UNPAUSED)           *pi++ = STR_NETWORK_SERVER_MESSAGE_GAME_REASON_CONNECTING_CLIENTS;
				if ((_pause_mode & PM_PAUSED_GAME_SCRIPT) != PM_UNPAUSED)    *pi++ = STR_NETWORK_SERVER_MESSAGE_GAME_REASON_GAME_SCRIPT;
				if ((_pause_mode & PM_PAUSED_ACTIVE_CLIENTS) != PM_UNPAUSED) *pi++ = STR_NETWORK_SERVER_MESSAGE_GAME_REASON_NOT_ENOUGH_PLAYERS;
				if ((_pause_mode & PM_PAUSED_LINK_GRAPH) != PM_UNPAUSED)     *pi++ = STR_NETWORK_SERVER_MESSAGE_GAME_REASON_LINK_GRAPH;
				str = GetStringWithArgs(STR_NETWORK_SERVER_MESSAGE_GAME_STILL_PAUSED_1 + std::distance(params.begin(), pi) - 1, {params.begin(), pi});
			} else {
				StringID reason;
				switch (changed_mode) {
