...
std::vector<uint8_t> _sorted_cargo_types; ///< Sort order of cargoes by cargo ID.
...
uint &CargoArray::operator[](CargoID cid)
{
	auto it = std::ranges::lower_bound(this->amounts, cid, std::less{}, &ElementType::first);
	if (it == std::end(this->amounts) || it->first != cid) it = this->amounts.emplace(it, cid, 0);
	return it->second;
}

bool CargoArray::Contains(CargoID cid) const
{
	auto it = std::ranges::lower_bound(this->amounts, cid, std::less{}, &ElementType::first);
	return it != std::end(this->amounts) && it->first == cid && it->second != 0;
}
...
std::optional<std::string> BuildCargoAcceptanceString(const CargoArray &acceptance, StringID label)
{
	...
	for (const CargoSpec *cs : _sorted_cargo_specs) {
		...
		auto it = std::ranges::lower_bound(acceptance.amounts, cid, std::less{}, &CargoArray::ElementType::first);
		if (it == std::end(acceptance.amounts) || it->first != cid || it->second == 0) continue;
