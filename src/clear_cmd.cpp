...
static void DrawClearLandFence(const TileInfo *ti)
{
	...
	if (fence_nw != 0) {
		...
		AddSortableSpriteToDraw(sprite, PAL_NONE, ti->x, ti->y - 15, 16, 16, maxz - z + 4, ti->z + z, false, 0, 15, -z);
	}
	...
	if (fence_ne != 0) {
		...
		AddSortableSpriteToDraw(sprite, PAL_NONE, ti->x - 15, ti->y, 16, 16, maxz - z + 4, ti->z + z, false, 15, 0, -z);
	}
