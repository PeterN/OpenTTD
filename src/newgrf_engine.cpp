	...
	switch (variable) {
		...
		case 0x42: { // Consist cargo information
			if (!HasBit(v->grf_cache.cache_valid, NCVV_CONSIST_CARGO_INFORMATION)) {
				...
				CargoID common_cargo_type = (*cargo_it == 0) ? INVALID_CARGO : static_cast<CargoID>(std::distance(common_cargoes.get(), cargo_it));
	...
	cached_relative_count(0)
{
	if (wagon_override == WO_SELF) {
		...
		if (this->root_spritegroup == nullptr) {
			...
			if (this->root_spritegroup == nullptr) e->grf_prop.GetSpriteGroup(SpriteGroupCargo::SG_DEFAULT);
		}
	}
}
