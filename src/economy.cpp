...
static void HandleEconomyFluctuations()
{
	...
	if (_economy.fluct == 0) {
		...
		AddNewsItem(GetEncodedString(STR_NEWS_BEGIN_OF_RECESSION), NT_ECONOMY, NF_NORMAL);
	} else if (_economy.fluct == -12) {
		_economy.fluct = GB(Random(), 0, 8) + 312;
		AddNewsItem(GetEncodedString(STR_NEWS_END_OF_RECESSION), NT_ECONOMY, NF_NORMAL);
	}
}
...
static void DoAcquireCompany(Company *c, bool hostile_takeover)
{
	...
	EncodedString headline = GetEncodedString(hostile_takeover ? STR_NEWS_MERGER_TAKEOVER_TITLE : STR_NEWS_COMPANY_MERGER_DESCRIPTION, cni->company_name, cni->other_company_name, c->bankrupt_value);
	AddCompanyNewsItem(std::move(headline), std::move(cni));
