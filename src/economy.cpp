...
int UpdateCompanyRatingAndValue(Company *c, bool update)
{
	...
	/* Generate score depending on amount of transported cargo */
	{
		...
		if (numec != 0) {
			const CompanyEconomyEntry *cee = c->old_economy;
			OverflowSafeint64_t total_delivered = 0;
			do {
				total_delivered += cee->delivered_cargo.GetSum<OverflowSafeint64_t>();
			} while (++cee, --numec);
...
{
	/* Setup maximum loan */
	_economy.max_loan = ((uint64_t)_settings_game.difficulty.max_loan * _economy.inflation_prices >> 16) / 50000 * 50000;
	...
	for (CargoSpec *cs : CargoSpec::Iterate()) {
		cs->current_payment = ((int64_t)cs->initial_payment * _economy.inflation_payment) >> 16;
	}
...
Money GetTransportedGoodsIncome(uint num_pieces, uint dist, byte transit_days, CargoID cargo_type)
{
	...
	if (HasBit(cs->callback_mask, CBM_CARGO_PROFIT_CALC)) {
		uint32_t var18 = std::min(dist, 0xFFFFu) | (std::min(num_pieces, 0xFFu) << 16) | (transit_days << 24);
 ...
 */
CommandCost CmdBuyShareInCompany(TileIndex tile, DoCommandFlag flags, uint32_t p1, uint32_t p2, const std::string &text)
{
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdSellShareInCompany(TileIndex tile, DoCommandFlag flags, uint32_t p1, uint32_t p2, const std::string &text)
{
	CompanyID target_company = (CompanyID)p1;
	Company *c = Company::GetIfValid(target_company);
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdBuyCompany(TileIndex tile, DoCommandFlag flags, uint32_t p1, uint32_t p2, const std::string &text)
{
	CompanyID target_company = (CompanyID)p1;
	Company *c = Company::GetIfValid(target_company);
