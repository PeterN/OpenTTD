...
static void HandleStationRefit(Vehicle *v, CargoArray &consist_capleft, Station *st, StationIDStack next_station, CargoID new_cid)
{
	...
	if (is_auto_refit) {
		...
		FOR_EACH_SET_CARGO_ID(cid, refit_mask) {
			if (st->goods[cid].cargo.HasCargoFor(next_station)) {
				...
				DoCommand(PackCmdRefitVehicle(v_start->index, v_start->type, cid, 0xFF, 1, true, false), DC_QUERY_COST); // Auto-refit and only this vehicle including artic parts.
	...
	if (new_cid < NUM_CARGO && new_cid != v_start->cargo_type) {
		...
		CommandCost cost = DoCommand(PackCmdRefitVehicle(v_start->index, v_start->type, new_cid, 0xFF, 1, true, false), DC_EXEC); // Auto-refit and only this vehicle including artic parts.
