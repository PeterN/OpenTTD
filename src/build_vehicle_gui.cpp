...
static bool CargoAndEngineFilter(const GUIEngineListItem *item, const CargoType cargo_type)
{
	if (cargo_type == CargoFilterCriteria::CF_ANY) {
		...
		return (cargo_type == CargoFilterCriteria::CF_NONE ? refit_mask.empty() : HasCargo(refit_mask, cargo_type));
	}
}
...
static int DrawCargoCapacityInfo(int left, int right, int y, TestedEngineDetails &te, bool refittable)
{
	for (const CargoSpec *cs : _sorted_cargo_specs) {
		...
		if (!te.all_capacities.Contains(cargo_type) || te.all_capacities[cargo_type] == 0) continue;
...
struct BuildVehicleWindow : Window {
	...
	void SetCargoFilterArray()
	{
		...
		if (this->cargo_filter_criteria < CargoSpec::Count() && !HasCargo(_standard_cargo_mask, this->cargo_filter_criteria)) this->cargo_filter_criteria = CargoFilterCriteria::CF_ANY;
