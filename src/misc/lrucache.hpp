...
class LRUCache {
private:
	typedef std::pair<const Tkey, Tdata *> Tpair;
	typedef typename std::list<Tpair>::iterator Titer;

	std::list<Tpair> data;               ///< Ordered list of all items.
	std::map<const Tkey, Titer> lookup;  ///< Map of keys to items.
...
public:
	 ...
	 * @return Evicted item or NULL, if no item had to be evicted.
	 */
	Tdata *Insert(const Tkey key, Tdata *item)
	{
		...
		if (this->Contains(key)) {
			...
			this->lookup[key] = this->data.begin();
		}
	 ...
	 * @return The item value or NULL if no items cached.
	 */
	inline Tdata *Pop()
	{
	...
	inline Tdata *Get(const Tkey key)
	{
		...
		this->data.splice(this->data.begin(), this->data, this->lookup.at(key));
