...
static inline CargoTypes GetAvailableVehicleCargoTypes(EngineID engine, bool include_initial_cargo_type)
{
	...
	if (include_initial_cargo_type) {
		SetCargo(cargoes, e->GetDefaultCargoType());
	}
...
void GetArticulatedRefitMasks(EngineID engine, bool include_initial_cargo_type, CargoTypes *union_mask, CargoTypes *intersection_mask)
{
	...
	*intersection_mask = (veh_cargoes.empty()) ? ~CargoTypes{} : veh_cargoes;
	...
	for (uint i = 1; i < MAX_ARTICULATED_PARTS; i++) {
		...
		*union_mask |= veh_cargoes;
		if (!veh_cargoes.empty()) *intersection_mask &= veh_cargoes;
	}
}
...
void CheckConsistencyOfArticulatedVehicle(const Vehicle *v)
{
	...
	CargoTypes real_refit_intersection = ~CargoTypes{};
	...
	do {
		...
		if (!refit_mask.empty()) real_refit_intersection &= refit_mask;

		assert(IsValidCargoType(v->cargo_type));
		if (v->cargo_cap > 0) SetCargo(real_default_cargoes, v->cargo_type);
	...
	for (const CargoType &cargo_type : real_default_cargoes) {
		if (!purchase_default_capacity.Contains(cargo_type)) {
			carries_more = true;
			break;
		}
	}
