 ...
 * @param flags StringSpriteToDrawFlags to control the string's appearance.
 * @param width Width of the string.
 * @returns Reference to raw string which should be filled in by the caller.
 */
static std::string &AddStringToDraw(int x, int y, Colours colour, uint8_t flags, uint16_t width)
{
	assert(width != 0);
 ...
 */
std::string *ViewportAddString(const DrawPixelInfo *dpi, const ViewportSign *sign, uint8_t flags, Colours colour)
{
	int left   = dpi->left;
	int top    = dpi->top;
	...
	bool small = HasBit(flags, SSTDF_SMALL);
...
static void ViewportAddTownStrings(DrawPixelInfo *dpi, const std::vector<const Town *> &towns, bool small)
{
	uint8_t flags = 0;
	if (small) {
		SetBit(flags, SSTDF_SMALL);
		SetBit(flags, SSTDF_SHADOW);
	}
...
{
	uint8_t flags = 0;
	if (small) SetBit(flags, SSTDF_SMALL);

	/* Signs placed by a game script don't have a frame at all. */
	uint8_t deity_flags = flags;
	if (IsTransparencySet(TO_SIGNS)) {
		SetBit(flags, SSTDF_TRANS_RECT);
	} else {
		SetBit(flags, SSTDF_COLOUR_RECT);
	}
...
static void ViewportAddStationStrings(DrawPixelInfo *dpi, const std::vector<const BaseStation *> &stations, bool small)
{
	...
	if (IsTransparencySet(TO_SIGNS)) {
		SetBit(flags, SSTDF_TEXT_COLOUR);
	} else {
		SetBit(flags, SSTDF_COLOUR_RECT);
	}
...
static void ViewportDrawStrings(ZoomLevel zoom, const StringSpriteToDrawVector *sstdv)
{
	for (const StringSpriteToDraw &ss : *sstdv) {
		bool small = HasBit(ss.flags, SSTDF_SMALL);
		int w = ss.width;
		...
		int shadow_offset = 0;
		if (small && HasBit(ss.flags, SSTDF_SHADOW)) {
