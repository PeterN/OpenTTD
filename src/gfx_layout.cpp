...
static inline void GetLayouter(Layouter::LineCacheItem &line, std::string_view str, FontState &state)
{
	...
	for (; buff < buffer_last && cur != str.end();) {
		...
		if (buff - buff_begin > 0 && fontMapping.count(buff - buff_begin) == 0) {
	...
	*buff = '\0';
	...
	if constexpr (!std::is_same_v<T, FallbackParagraphLayoutFactory>) {
		...
		if (std::all_of(std::begin(fontMapping), std::end(fontMapping), [](const auto &i) { return i.second->fc->IsBuiltInFont(); })) {
			return;
		}
	}
