		...
		switch (widget) {
			case WID_O_COND_VALUE: {
				...
				if (order != nullptr && order->IsType(OT_CONDITIONAL)) {
					...
					param.SetParam(0, value);
				}
				break;
			}

			case WID_O_CAPTION:
				param.SetParam(0, this->vehicle->index);
				break;
			...
			case WID_O_DEPOT_ACTION: {
				...
				if (order->GetDepotOrderType() & ODTFB_SERVICE) {
					param.SetParam(0, STR_ORDER_DROP_SERVICE_DEPOT);
				} else if (order->GetDepotActionType() & ODATFB_HALT) {
					param.SetParam(0, STR_ORDER_DROP_HALT_DEPOT);
				} else if (order->GetDepotActionType() & ODATFB_UNBUNCH) {
					param.SetParam(0, STR_ORDER_DROP_UNBUNCH);
				} else {
					param.SetParam(0, STR_ORDER_DROP_GO_ALWAYS_DEPOT);
				}
				break;
			}
		}
