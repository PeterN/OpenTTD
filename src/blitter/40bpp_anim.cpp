...
void Blitter_40bppAnim::SetPixel(void *video, int x, int y, uint8_t colour)
{
	if (_screen_disable_anim) {
		...
		*((Colour *)video + x + y * _screen.pitch) = _black_colour;
		...
		VideoDriver::GetInstance()->GetAnimBuffer()[((uint32_t *)video - (uint32_t *)_screen.dst_ptr) + x + y * _screen.pitch] = colour;
	}
}
...
void Blitter_40bppAnim::DrawColourMappingRect(void *dst, int width, int height, PaletteID pal)
{
	...
	if (pal == PALETTE_TO_TRANSPARENT) {
		...
		const uint8_t *remap = GetNonSprite(pal, ST_RECOLOUR) + 1;
		do {
			for (int i = 0; i != width; i++) {
...
{
	return width * height * (sizeof(uint32_t) + sizeof(uint8_t));
}
