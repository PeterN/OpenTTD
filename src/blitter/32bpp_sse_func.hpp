...
static inline void InsertFirstUint32_t(const uint32_t value, __m128i &into)
{
#if (SSE_VERSION >= 4)
...
static inline void InsertSecondUint32_t(const uint32_t value, __m128i &into)
{
#if (SSE_VERSION >= 4)
...
static inline void LoadUint64_t(const uint64_t value, __m128i &into)
{
	...
	into = _mm_cvtsi64_si128(value);
		...
		InsertSecondUint32_t(value >> 32, into);
	#else
...
{
	__m128i srcAB = _mm_unpacklo_epi8(src, _mm_setzero_si128());   // PUNPCKLBW, expand each uint8_t into uint16_t
