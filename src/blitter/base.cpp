...
void Blitter::DrawLine(void *video, int x, int y, int x2, int y2, int screen_width, int screen_height, uint8 colour, Colour rgb, int width, int dash)
{
	int dy;
	int dx;
	int stepx;
	...
	if (dx == 0 && dy == 0) {
		...
		if (x >= 0 && x < screen_width && y >= 0 && y < screen_height) this->SetPixel(video, x, y, colour, rgb);
		return;
	}
		...
		while (x != x2) {
			if (dash_count < dash && x >= 0 && x < screen_width) {
				for (int y = y_low; y != y_high; y += stepy) {
					if (y >= 0 && y < screen_height) this->SetPixel(video, x, y, colour, rgb);
				}
			}
			if (frac_low >= 0) {
				y_low += stepy;
		...
		while (y != y2) {
			if (dash_count < dash && y >= 0 && y < screen_height) {
				for (int x = x_low; x != x_high; x += stepx) {
					if (x >= 0 && x < screen_width) this->SetPixel(video, x, y, colour, rgb);
				}
			}
			if (frac_low >= 0) {
				x_low += stepx;
