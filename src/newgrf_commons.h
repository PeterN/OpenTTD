...
struct EntityIDMapping {
	...
	uint8_t  entity_id;      ///< The entity ID within the GRF file
	uint8_t  substitute_id;  ///< The (original) entity ID to use if this GRF is not available
};

class OverrideManagerBase {
protected:
	uint16_t *entity_overrides;
	uint32_t *grfid_overrides;

	uint16_t max_offset;       ///< what is the length of the original entity's array of specs
	uint16_t max_new_entities; ///< what is the amount of entities, old and new summed
...
public:
	...
	void Add(uint8_t local_id, uint32_t grfid, uint entity_type);
	virtual uint16_t AddEntityID(byte grf_local_id, uint32_t grfid, byte substitute_id);

	uint32_t GetGRFID(uint16_t entity_id) const;
	uint16_t GetSubstituteID(uint16_t entity_id) const;
	virtual uint16_t GetID(uint8_t grf_local_id, uint32_t grfid) const;

	inline uint16_t GetMaxMapping() const { return max_new_entities; }
	inline uint16_t GetMaxOffset() const { return max_offset; }
};
...
public:
	...
	uint16_t AddEntityID(byte grf_local_id, uint32_t grfid, byte substitute_id) override;
	uint16_t GetID(uint8_t grf_local_id, uint32_t grfid) const override;
