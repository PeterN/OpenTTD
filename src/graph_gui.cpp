 ...
 */
static NWidgetBase *MakeNWidgetCompanyLines(WidgetIndex *biggest_index)
{
	...
	for (WidgetIndex widnum = WID_GL_FIRST_COMPANY; widnum <= WID_GL_LAST_COMPANY; widnum++) {
...
protected:
	...
	BaseGraphWindow(WindowDesc *desc, WidgetIndex widget, StringID format_str_y_axis) :
			Window(desc),
 ...
 * @post \c *biggest_index contains the largest used index in the tree.
 */
static NWidgetBase *MakePerformanceDetailPanels(WidgetIndex *biggest_index)
{
	...
	NWidgetVertical *vert = new NWidgetVertical(NC_EQUALSIZE);
	for (WidgetIndex widnum = WID_PRD_SCORE_FIRST; widnum <= WID_PRD_SCORE_LAST; widnum++) {
		NWidgetBackground *panel = new NWidgetBackground(WWT_PANEL, COLOUR_BROWN, widnum);
		panel->SetFill(1, 1);
...
NWidgetBase *MakeCompanyButtonRowsGraphGUI(WidgetIndex *biggest_index)
{
