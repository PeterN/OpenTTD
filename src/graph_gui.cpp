...
struct GraphLegendWindow : Window {
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		DrawString(tr.left, tr.right, CenterBounds(tr.top, tr.bottom, GetCharacterHeight(FS_NORMAL)), GetString(STR_COMPANY_NAME_COMPANY_NUM, cid, cid), HasBit(_legend_excluded_companies, cid) ? TC_BLACK : TC_WHITE);
	}
	...
	void DrawGraph(Rect r) const
	{
		...
		if (this->draw_dates) {
			...
			for (int i = 0; i < this->num_on_x_axis; i++) {
				DrawStringMultiLine(x, x + x_sep, y, this->height, GetString(month == 0 ? STR_GRAPH_X_LABEL_MONTH_YEAR : STR_GRAPH_X_LABEL_MONTH, month + STR_MONTH_ABBREV_JAN, year), GRAPH_AXIS_LABEL_COLOUR, SA_LEFT);
		...
		switch (widget) {
			...
			case WID_GRAPH_GRAPH: {
				...
				if (this->draw_dates) {
					...
					TimerGameEconomy::Year year = this->year;
					for (int i = 0; i < this->num_on_x_axis; i++) {
						x_label_width = std::max(x_label_width, GetStringBoundingBox(GetString(month == 0 ? STR_GRAPH_X_LABEL_MONTH_YEAR : STR_GRAPH_X_LABEL_MONTH, month + STR_MONTH_ABBREV_JAN, year)).width);
