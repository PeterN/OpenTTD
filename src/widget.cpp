...
static std::string GetStringWithParameters(const Window *w, const NWidgetCore *nwid, bool secondary = false)
{
	if (nwid->widget_data == STR_NULL) return {};

	if (nwid->index >= 0) {
		/* We use static storage for widget string parameters so reduce allocations. */
		static ArrayStringParametersWriter<20> params;
		std::ranges::fill(params, StringParameter{});

		w->SetStringParameters(nwid->index, params);
		return GetStringWithArgs(nwid->widget_data + (secondary ? 1 : 0), params);
	}

	return GetString(nwid->widget_data + (secondary ? 1 : 0));
}
...
void NWidgetBackground::SetupSmallestSize(Window *w)
{
	if (this->child != nullptr) {
		...
		if (this->type == WWT_FRAME) {
			...
			std::string text = GetStringWithParameters(w, this);
...
void NWidgetBackground::Draw(const Window *w)
{
	...
	switch (this->type) {
		...
		case WWT_FRAME:
			DrawFrame(r, this->colour, this->text_colour, GetStringWithParameters(w, this), this->align, this->text_size);
			break;

		case WWT_INSET:
			DrawInset(r, this->colour, this->text_colour, GetStringWithParameters(w, this), this->align, this->text_size);
			break;
...
void NWidgetLeaf::SetupSmallestSize(Window *w)
{
	...
	switch (this->type) {
		...
		case WWT_TEXTBTN_2: {
			...
			Dimension d2 = GetStringBoundingBox(GetStringWithParameters(w, this), this->text_size);
			d2.width += padding.width;
			d2.height += padding.height;
			size = maxdim(size, d2);
			break;
		...
		case WWT_LABEL:
		case WWT_TEXT: {
			size = maxdim(size, GetStringBoundingBox(GetStringWithParameters(w, this), this->text_size));
			break;
		}
		case WWT_CAPTION: {
...
void NWidgetLeaf::Draw(const Window *w)
{
	...
	switch (this->type) {
		...
		case WWT_TEXTBTN_2:
			DrawFrameRect(r.left, r.top, r.right, r.bottom, this->colour, (clicked) ? FR_LOWERED : FR_NONE);
			DrawLabel(r, this->text_colour, GetStringWithParameters(w, this, (type & WWT_MASK) == WWT_TEXTBTN_2 && clicked), this->align, this->text_size);
			break;
		...
		case WWT_LABEL:
			DrawLabel(r, this->text_colour, GetStringWithParameters(w, this), this->align, this->text_size);
			break;

		case WWT_TEXT:
			DrawText(r, this->text_colour, GetStringWithParameters(w, this), this->align, this->text_size);
			break;
		...
		case WWT_CAPTION:
			DrawCaption(r, this->colour, w->owner, this->text_colour, GetStringWithParameters(w, this), this->align, this->text_size);
			break;
		...
		case WWT_DROPDOWN:
			DrawButtonDropdown(r, this->colour, false, clicked, GetStringWithParameters(w, this), this->align);
			break;

		case NWID_BUTTON_DROPDOWN:
		case NWID_PUSHBUTTON_DROPDOWN:
			DrawButtonDropdown(r, this->colour, clicked, (this->disp_flags & ND_DROPDOWN_ACTIVE) != 0, GetStringWithParameters(w, this), this->align);
			break;
