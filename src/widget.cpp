...
{
	uint dark         = ShadeColour(colour, 3);
	uint medium_dark  = ShadeColour(colour, 5);
	uint medium_light = ShadeColour(colour, 6);
	uint light        = ShadeColour(colour, 7);
...
static inline void DrawVerticalScrollbar(const Rect &r, Colours colour, bool up_clicked, bool bar_dragged, bool down_clicked, const Scrollbar *scrollbar)
{
	...
	int c1 = ShadeColour(colour, 3);
	int c2 = ShadeColour(colour, 7);
...
void DrawCaption(const Rect &r, Colours colour, Owner owner, TextColour text_colour, StringID str, StringAlignment align)
{
	...
	if (company_owned) {
		GfxFillRect(ir.Shrink(WidgetDimensions::scaled.bevel), ShadeColour(_company_colours[owner], 4));
	}
...
{
	/* Fill the background. */
	GfxFillRect(this->pos_x, this->pos_y, this->pos_x + this->current_x - 1, this->pos_y + this->current_y - 1, ShadeColour(this->colour, 5));
...
void NWidgetBackground::Draw(const Window *w)
{
	...
	if (this->IsDisabled()) {
		GfxFillRect(r.Shrink(WidgetDimensions::scaled.bevel), ShadeColour(this->colour, 2), FILLRECT_CHECKER);
	}
}
