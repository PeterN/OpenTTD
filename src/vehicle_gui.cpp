...
struct RefitWindow : public Window {
	...
	{
		if (widget == WID_VR_CAPTION) param.SetParam(0, Vehicle::Get(this->window_number)->index);
	}
	...
	std::string GetCapacityString(const RefitOption &option) const
	{
		...
		params.SetParam(0, option.cargo);
		params.SetParam(1, refit_capacity);

		Money money = cost.GetCost();
		if (mail_capacity > 0) {
			params.SetParam(2, GetCargoIDByLabel(CT_MAIL));
			params.SetParam(3, mail_capacity);
			if (this->order != INVALID_VEH_ORDER_ID) {
				/* No predictable cost */
				str = STR_PURCHASE_INFO_AIRCRAFT_CAPACITY;
			} else if (money <= 0) {
				params.SetParam(4, -money);
				str = STR_REFIT_NEW_CAPACITY_INCOME_FROM_AIRCRAFT_REFIT;
			} else {
				params.SetParam(4, money);
				str = STR_REFIT_NEW_CAPACITY_COST_OF_AIRCRAFT_REFIT;
			}
		} else {
			if (this->order != INVALID_VEH_ORDER_ID) {
				/* No predictable cost */
				params.SetParam(2, STR_EMPTY);
				str = STR_PURCHASE_INFO_CAPACITY;
			} else if (money <= 0) {
				params.SetParam(2, -money);
				str = STR_REFIT_NEW_CAPACITY_INCOME_FROM_REFIT;
			} else {
				params.SetParam(2, money);
				str = STR_REFIT_NEW_CAPACITY_COST_OF_REFIT;
			}
		}
...
uint ShowRefitOptionsList(int left, int right, int y, EngineID engine)
{
	...
	if (cmask == lmask) {
		...
		str = GetString(STR_PURCHASE_INFO_REFITTABLE_TO, STR_PURCHASE_INFO_ALL_TYPES);
	} else {
				...
				if (_settings_client.gui.show_cargo_in_vehicle_lists) {
					...
					if (!v->name.empty()) {
						...
						DrawString(tr.left, tr.right, ir.top,
								GetString(STR_VEHICLE_LIST_NAME_AND_CARGO,
										STR_VEHICLE_NAME,
										v->index,
	...
	void SetStringParameters(WidgetID widget, WidgetStringParameters &param) const override
	{
		switch (widget) {
			case WID_VL_AVAILABLE_VEHICLES:
				param.SetParam(0, STR_VEHICLE_LIST_AVAILABLE_TRAINS + this->vli.vtype);
				break;

			case WID_VL_FILTER_BY_CARGO:
				param.SetParam(0, this->GetCargoFilterLabel(this->cargo_filter_criteria));
				break;
			...
			case WID_VL_CAPTION_SHARED_ORDERS: {
				switch (this->vli.type) {
					case VL_SHARED_ORDERS: // Shared Orders
						param.SetParam(0, this->vehicles.size());
						break;

					case VL_STANDARD: // Company Name
						param.SetParam(0, STR_COMPANY_NAME);
						param.SetParam(1, this->vli.index);
						param.SetParam(3, this->vehicles.size());
						break;

					case VL_STATION_LIST: // Station/Waypoint Name
						param.SetParam(0, Station::IsExpected(BaseStation::Get(this->vli.index)) ? STR_STATION_NAME : STR_WAYPOINT_NAME);
						param.SetParam(1, this->vli.index);
						param.SetParam(3, this->vehicles.size());
						break;

					case VL_DEPOT_LIST:
						param.SetParam(0, STR_DEPOT_CAPTION);
						param.SetParam(1, this->vli.vtype);
						param.SetParam(2, this->vli.index);
						param.SetParam(3, this->vehicles.size());
						break;
					default: NOT_REACHED();
				}
...
struct VehicleDetailsWindow : Window {
	...
	void UpdateWidgetSize(WidgetID widget, Dimension &size, [[maybe_unused]] const Dimension &padding, [[maybe_unused]] Dimension &fill, [[maybe_unused]] Dimension &resize) override
	{
		switch (widget) {
			case WID_VD_TOP_DETAILS: {
				...
				auto params = MakeParameters(max_value, max_value, max_value, max_value);
				...
				for (const auto &info_string : info_strings) {
					dim = maxdim(dim, GetStringBoundingBox(GetStringWithArgs(info_string, params)));
				}
				dim = maxdim(dim, GetStringBoundingBox(GetString(TimerGameEconomy::UsingWallclockUnits() ? STR_VEHICLE_INFO_AGE_RUNNING_COST_PERIOD : STR_VEHICLE_INFO_AGE_RUNNING_COST_YR, STR_VEHICLE_INFO_AGE)));
			...
			case WID_VD_SERVICING_INTERVAL:
				...
				auto params = MakeParameters(
					GetParamMaxValue(MAX_SERVINT_DAYS), // Roughly the maximum interval
					wallclock
						? STR_VEHICLE_DETAILS_LAST_SERVICE_MINUTES_AGO
						: STR_VEHICLE_DETAILS_LAST_SERVICE_DATE,
					wallclock
						? EconomyTime::MONTHS_IN_YEAR * EconomyTime::MAX_YEAR.base()
						: TimerGameEconomy::DateAtStartOfYear(EconomyTime::MAX_YEAR));
				size.width = std::max(
					GetStringBoundingBox(GetStringWithArgs(STR_VEHICLE_DETAILS_SERVICING_INTERVAL_PERCENT, params)).width,
					GetStringBoundingBox(GetStringWithArgs(STR_VEHICLE_DETAILS_SERVICING_INTERVAL_DAYS, params)).width
				) + padding.width;
	...
	void SetStringParameters(WidgetID widget, WidgetStringParameters &param) const override
	{
		if (widget == WID_VD_CAPTION) param.SetParam(0, Vehicle::Get(this->window_number)->index);
	}
			...
			case WID_VD_TOP_DETAILS: {
				...
				if (v->type == VEH_TRAIN ||
					...
					if (v->type == VEH_TRAIN && (_settings_game.vehicle.train_acceleration_model == AM_ORIGINAL ||
						...
						DrawString(tr,
							GetString(STR_VEHICLE_INFO_WEIGHT_POWER_MAX_SPEED,
								gcache->cached_weight,
								gcache->cached_power,
								max_speed));
					} else {
						DrawString(tr,
							GetString(STR_VEHICLE_INFO_WEIGHT_POWER_MAX_SPEED_MAX_TE,
								gcache->cached_weight,
								gcache->cached_power,
								max_speed,
					...
					if (Aircraft::From(v)->GetRange() > 0) {
						DrawString(tr, GetString(STR_VEHICLE_INFO_MAX_SPEED_TYPE_RANGE,
								max_speed, type, Aircraft::From(v)->GetRange()));
					} else {
						DrawString(tr, GetString(STR_VEHICLE_INFO_MAX_SPEED_TYPE,
								max_speed, type));
					}
				} else {
					DrawString(tr, GetString(STR_VEHICLE_INFO_MAX_SPEED,
							max_speed));
				}
				tr.top += GetCharacterHeight(FS_NORMAL);
...
public:
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		Vehicle *v = Vehicle::Get(this->window_number);
		ArrayStringParametersWriter<3> params; /* This function is too messy to do proper parameter ownership... */
		StringID str;
		...
		if (v->vehstatus & VS_CRASHED) {
			...
			if (v->type == VEH_TRAIN) {
				if (v->cur_speed == 0) {
					...
					params.SetParam(0, PackVelocity(v->GetDisplaySpeed(), v->type));
			...
			switch (v->current_order.GetType()) {
				case OT_GOTO_STATION: {
					params.SetParam(0, v->current_order.GetDestination());
					params.SetParam(1, PackVelocity(v->GetDisplaySpeed(), v->type));
				...
				case OT_GOTO_DEPOT: {
					params.SetParam(0, v->type);
					params.SetParam(1, v->current_order.GetDestination());
					params.SetParam(2, PackVelocity(v->GetDisplaySpeed(), v->type));
		...
		DrawString(tr.left, tr.right, CenterBounds(tr.top, tr.bottom, GetCharacterHeight(FS_NORMAL)), GetStringWithArgs(str, params), text_colour, SA_HOR_CENTER);
	}
