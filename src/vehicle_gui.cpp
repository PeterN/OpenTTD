...
struct RefitWindow : public Window {
	...
	std::string GetCapacityString(const RefitOption &option) const
	{
		...
		*pi++ = refit_capacity;
		...
		if (mail_capacity > 0) {
			*pi++ = GetCargoTypeByLabel(CT_MAIL);
			*pi++ = mail_capacity;
		...
		return GetStringWithArgs(str, {params.begin(), pi});
	}
...
public:
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		Vehicle *v = Vehicle::Get(this->window_number);
		std::array<StringParameter, 3> params{}; /* This function is too messy to do proper parameter ownership... */
		StringID str;
		...
		if (v->vehstatus & VS_CRASHED) {
			...
			if (v->type == VEH_TRAIN) {
				if (v->cur_speed == 0) {
					...
					params[0] = PackVelocity(v->GetDisplaySpeed(), v->type);
			...
			switch (v->current_order.GetType()) {
				case OT_GOTO_STATION: {
					params[0] = v->current_order.GetDestination();
					params[1] = PackVelocity(v->GetDisplaySpeed(), v->type);
				...
				case OT_GOTO_DEPOT: {
					params[0] = v->type;
					params[1] = v->current_order.GetDestination();
					params[2] = PackVelocity(v->GetDisplaySpeed(), v->type);
